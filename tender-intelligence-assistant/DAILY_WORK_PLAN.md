# Nova Minds — Capstone Sprint Plan

**Team Name:** Nova Minds  
**Repository:** https://github.com/capstone-nova-minds/main-repo  
**Project Board:** https://github.com/orgs/capstone-nova-minds/projects/3  

This file tracks the daily work plan, task ownership, expected outputs, and completion status during the Capstone sprint.

---

## 1. Sprint Goals

By the end of the sprint, our team aims to deliver:

- A working AI capstone project.
- A clean GitHub repository with documentation.
- A tested pipeline with clear evaluation results.
- A demo video or live demo.
- A presentation deck for Demo Day.
- A public portfolio artifact.

---


## 2. 5-July / Sunday's Working Block Agenda

| Time | Activity | Notes / Updates | Completed |
|---|---|---|---|
| 20 min | Intros round 2: strengths, growth areas, and working hours |  | ✅ |
| 40 min | Set up GitHub org, repo, Project board, and branch protection | GitHub setup done | ✅ |
| 30 min | Fill Cooperation Plan + Communication Plan in Team Contract |  | ✅ |
| 20 min | Break |  | ✅ |
| 30 min | Fill Conflict Plan + Work Plan + Git Process in Team Contract |  | ✅ |
| 15 min | Read contract aloud, sign off, commit, and push |  | ✅ |


## 3. 6-July / Monday Working Block Agenda

| Time | Activity | Notes / Updates | Completed |
|---|---|---|---|
| 2 hours | Check Team Contract status and confirm all sign-offs |  | ✅ |
| 60 min | Commit and push final `TEAM_CONTRACT.md` if not already done |  | ✅ |
| 10 min | Submit Team Contract link to TalentLMS |  | ✅ |
| 3 hours | Review 4–6 project ideas from the team |  | ✅ |
| 60 min | Compare ideas using scope, data availability, AI capability, and evaluation |  | ✅ |
| 60 min | Select one final project idea |  | ✅ |
| 60 min | Define problem statement and target user |  | ✅ |
| 60 min | Define primary AI capability and AISPIRE components |  | ✅ |
| 20 min | Assign proposal-writing sections to teammates |  | ✅ |
| 60 min | Brainstorm project ideas — aim for 4–6 candidates |  | ✅ |
| 30 min | Choose one idea and sanity-check scope |  | ✅ |