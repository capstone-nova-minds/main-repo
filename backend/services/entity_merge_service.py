...
def _enrich_short_rule_names_with_ner(
    ...
    """
    If a rule-based person has National ID but only 2 words,
    enrich it using NER PERSON suggestion.

    Example:
    Rules produced:
    يوسف الخوالدة + 98123456789
    ...
    Still marked needs_review=True because this is assisted reconstruction.
    """
    ...
    for person in merged_persons:
        ...
        if (
            ...
            for ner_text in person_ner_texts:
                candidate = _combine_ner_with_short_rule_name(full_name, ner_text)

                if candidate and (
                    best_name is None
                    or len(candidate.split()) > len(best_name.split())
                ):
                    best_name = candidate

            if best_name:
                person = _copy_person_with_updates(
                    person,
                    full_name=best_name,
