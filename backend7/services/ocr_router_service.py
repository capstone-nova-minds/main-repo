...
def run_ocr_on_page(page_number: int, image_path: Path) -> Dict[str, Any]:
    ...
    if needs_fallback:
        ...
        paddleocr_attempt = _run_engine_attempt(_paddleocr_engine, str(image_path))
        attempts.append(paddleocr_attempt)

        if paddleocr_attempt["quality_score"] > best_attempt["quality_score"]:
            best_attempt = paddleocr_attempt
    else:
        attempts.append({
        ...
        attempts.append({
            "engine": _paddleocr_engine.engine_name,
            "status": "skipped",
            "average_confidence": 0.0,
            "quality_score": 0.0,
            "text_length": 0,
